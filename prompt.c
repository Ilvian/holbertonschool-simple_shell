...
void prompt(void)
/*getline function reads user input and stores in text,allocates memory for the input buffer
 * if is the input is exit program closes with status 0
 * if input is env it iterates through the environment variables and prints specific ones
 * if code then forks a child using fork function ,child process runs indetify_string 
 * we use wait to make the part wait till child process is complted
 * for differen error print differe errors
 *
 */
{
	for (;;)
